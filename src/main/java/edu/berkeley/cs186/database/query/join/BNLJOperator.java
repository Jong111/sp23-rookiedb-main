        ...
        private Record fetchNextRecord() {
            ...
            while (true) {
                if (this.rightPageIterator.hasNext()) {
                    // there's a next right record in current page, join it if there's a match
                    Record rightRecord = rightPageIterator.next();
                    if (compare(leftRecord, rightRecord) == 0) {
                        return leftRecord.concat(rightRecord);
                    }
                } else if (leftBlockIterator.hasNext()) {
                    // there's no more right records in the current page but there's still left
                    // records int the current block. Advance left and reset right
